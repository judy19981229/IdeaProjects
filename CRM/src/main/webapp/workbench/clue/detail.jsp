...
<script type="text/javascript">
	...
	$(function(){
		...
		//关联市场活动，先搜索所有的市场活动，再关联
		//为关联市场活动模态窗口中的搜索框绑定事件，通过触发回车键来搜索市场活动
		$("#activityName").keydown(function(event){
			if(event.keyCode==13){
				//如果是回车键，查询并展现市场活动列表
				//查询出的是没有关联该线索的市场活动
				$.get("workbench/clue/getActivityListByName",
						{"name": $.trim($("#activityName").val()),
						"clueId":"${clue.id}"}, rollback,"json");
				function rollback(param){//param是一个市场活动的数组
					var html="";
				...
				//但是在模态窗口中，只要按回车就会自动刷新并关闭模态窗口,需要禁用该功能
				return false; //强行终止
			}
		});
		...
		$("#bundBtn").click(function(){
			$select=$(":checkbox[name=select]:checked");
			if($select.length==0){
				alert("请选择需要关联的市场活动");
			}else{
				var data="clueId=${clue.id}";
				for(var i=0;i<$select.length;i++){
					data+="&activityId="+$($select[i]).val();
				}
				$.get("workbench/clue/bund",data,rollback,"json");
				function rollback(param){
					if(param.success){
						$("#activityName").val("");//清空搜索框
						$("#activitySearchBody").html("");//清空搜索内容
						$("#bundModal").modal("hide");//关闭模态窗口
						showActivityList();//刷新关联的市场活动列表
					}else{alert("关联市场活动失败")}
				}
			}
		});
		//关联市场活动复选框的全选和全不选
		$("#activitySearchBody").on("click",$(":checkbox[name=select]"),
			function(){
				var num=$(":checkbox[name=select]").length
	...
	function showActivityList(){
		$.get("workbench/clue/getActivityListByClueId","clueId=${clue.id}",
				function(param){
					//param是一个一个的市场活动,存的id最好是市场活动关联表的id，方便后面解除关联关系
					var html="";
					$.each(param,function(index,dmoObj){
						html+='<tr>';
						html+='<td>'+dmoObj.name+'</td>';
						...
						html+='<td>'+dmoObj.endDate+'</td>';
						html+='<td>'+dmoObj.owner+'</td>';
						html+='<td><a href="javascript:void(0);" onclick="unbund(\''+dmoObj.id+'\')" style="text-decoration:none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
						html+='</tr>';
					});
	...
	function unbund(id){
		$.get("workbench/clue/deleteClue","id="+id,rollback,"json");
		function rollback(param){
			if(param.success){
				//刷新关联的市场活动列表
				showActivityList();
			}else{alert("解除关联失败，请重试")}
		}
	}
